 ...
 *  settings-derived values (typography, spacing, and the colors) a stylesheet
 *  can read with `var(…)`. The generated rules set their values directly, so
 *  redefining one changes nothing — they are a read contract. */
 ...
 * Stylesheet tab links to. Generated from the two lists above, so the
 * rendered section and the drift-tested contract are the same data.
 */
...
The generated values are mirrored as custom properties on \`.mpdf-doc\`. Read
them to align your own rules with the Document's current style values —
redefining one changes nothing, because the generated rules set their values
directly.
...
Every content selector is scoped under the document root, \`.mpdf-doc\`.

${entryTable(STYLING_REFERENCE_SELECTORS, 'Selector')}

### What is not CSS

Page geometry — paper size, orientation, and margins — the paper background,
and the header/footer bands are Inspector settings (the Page and Header/Footer
tabs), not CSS. \`@page\` rules are ignored: they are stripped from the
stylesheet before it is applied, because a rule obeyed in the print only would
silently split the preview from the PDF.`;
}
