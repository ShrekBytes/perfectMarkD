 ...
 *  when the band has no text at all. */
...
export function buildExportHTML(
    ...
    ? (() => {
        ...
        const pos =
          s.backgroundImageScope === 'content-only'
            ...
            : 'inset:0;';
        const common = `background-image:url('${bgUrl}');background-size:${bgCss.size};background-repeat:${bgCss.repeat};background-position:center;opacity:${s.backgroundImageOpacity};pointer-events:none;`;
  ...
  const pageHTMLParts = layouts.map((layout) => {
    ...
    const headerBorder = s.showHeaderBorder
      ? `border-bottom:0.5px solid ${s.accentColor}33;`
      : '';
    const headerHTML = layout.hasHeader
      ? `<div style="position:absolute;top:${g.mTop * 0.4}px;left:${g.mLeft}px;right:${g.mRight}px;height:${g.headerH}px;display:flex;align-items:center;font-size:${s.headerFontSize}px;color:${s.headerFontColor};font-family:${fontFamily};white-space:nowrap;${headerBorder}">${buildHFInnerHTML(layout.headerCenter, layout.headerLeft, layout.headerRight)}</div>`
      : '';
    ...
    const footerBorder = s.showFooterBorder
      ? `border-top:0.5px solid ${s.accentColor}33;`
      : '';
    const footerHTML = layout.hasFooter
      ? `<div style="position:absolute;bottom:0;left:0;right:0;height:${g.footerH}px;display:flex;align-items:center;${footerBorder}padding:0 ${g.mRight}px 0 ${g.mLeft}px;font-size:${s.footerFontSize}px;color:${s.footerFontColor};font-family:${fontFamily};">${buildHFInnerHTML(layout.footerCenter, layout.footerLeft, layout.footerRight)}</div>`
      : '';
    ...
    const headerBannerHTML =
      layout.pageShowsHeader && headerBannerUrl
        ? `<div style="position:absolute;top:${g.mTop * 0.4}px;left:${s.headerImageMargin}px;right:${s.headerImageMargin}px;height:${g.headerH}px;background-image:url('${headerBannerUrl}');background-size:cover;background-position:center;background-repeat:no-repeat;pointer-events:none;"></div>`
        : '';
    const footerBannerHTML =
      layout.pageShowsFooter && footerBannerUrl
        ? `<div style="position:absolute;bottom:0;left:${s.footerImageMargin}px;right:${s.footerImageMargin}px;height:${g.footerH}px;background-image:url('${footerBannerUrl}');background-size:cover;background-position:center;background-repeat:no-repeat;pointer-events:none;"></div>`
        : '';
