...
TDD at the seams: HistoryStore (round-trip, at-rest encryption envelope, per-user keys, path safety, expiry), routes through `createApp` (gates, list, download, ownership, expiry, unmounted case), the worker hook, the purge, and the dialog. 900 workspace tests green (35 new); typecheck + lint clean. Verified the `size_bytes` migration applies on top of `0004`.
