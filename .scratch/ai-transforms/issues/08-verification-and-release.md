...
- [x] The whole workspace is green: unit suites, both browser suites, typecheck, and lint, with no skipped or quarantined tests standing in for a fix. — **True when this ticket closed, false now, and not because this ticket was wrong.** Verified at `5b3cd97`: `AiReviewDialog.tsx` existed and all ten `ai-review-dialog` references in the three AI specs matched it. Three commits later `035c7d8` replaced that dialog with `AiReviewBar` and the editor decorations, migrated the unit tests, and left the e2e specs behind — taking the browser suite red. Filed as [09](09-browser-suite-after-in-place-review.md).
