...
- [ ] The AI Provider Config panel shows the token equivalent of the input cap and the worst-case cost of one AI Action (spec §AI Provider Config — ticket 03's deferral lands here): the input cap converted through the shared estimator and the output cap as configured, both labelled as estimates, priced at the model's published rates when Test connection reports them, with the price flagged as unknown when it does not, so a pricey model is a visible decision.
