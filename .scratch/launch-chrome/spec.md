...
- **Follow-up required by the hosting change (ADR-0010, 2026-09-23)**: the Privacy page's "What is never done" section claims *"The fonts are self-hosted, so your browser talks to no one else — except when you submit an AI Action"*. Once the deployment is published through a Cloudflare Tunnel, every request — including a Server Export payload — traverses Cloudflare's edge, which terminates TLS, so "talks to no one else" is false and "your PDF content is never read, used, or shared" needs the same qualification. Cloudflare is infrastructure in the path, not an analytics or tracking service, and the page should say so plainly. The e2e smoke test asserts the current copy, so the edit and the test move together. Not part of this spec's original scope — recorded here because this spec owns the Privacy page's content.
- **Status flipped `ready-for-human` → `resolved`** (2026-09-24): implementation shipped (`3ba10b1`) and was verified in a real browser; nothing here is still waiting on a human. The ADR-0010 follow-up above is superseded — the Admin's 2026-09-23 hosting-privacy decision keeps the Privacy page silent on where and how the service is hosted, so the claim was narrowed to "no page here loads anything from another company's server" instead of gaining a Cloudflare disclosure.
